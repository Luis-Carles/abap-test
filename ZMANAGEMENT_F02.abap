...
" Subroutine that calcules only once the ids present in DB TABLE not in results table and
" store them in global variables.
FORM all_ids.
  DATA: lv_highest TYPE abap_bool VALUE abap_true.

  SELECT CLIENT_ID, ORDER_COUNT FROM ZCLIENTS INTO CORRESPONDING FIELDS OF TABLE @gt_client_ids.
    IF sy-subrc <> 0.
      lv_highest = abap_false.
    ELSE.
      SELECT MAX( CLIENT_ID ) FROM ZCLIENTS INTO gv_high_clid.
    ENDIF.

  SELECT PROD_ID FROM ZPRODUCTS INTO CORRESPONDING FIELDS OF TABLE @gt_prod_ids.
    IF sy-subrc <> 0.
      lv_highest = abap_false.
    ELSE.
      SELECT MAX( PROD_ID ) FROM ZPRODUCTS INTO @gv_high_prid.
    ENDIF.

  SELECT MAX( ORDER_ID ) FROM ZCORDERS INTO @gv_high_orid.
    IF sy-subrc <> 0.
      lv_highest = abap_false.
    ENDIF.

  IF lv_highest = abap_false.
    MESSAGE 'Error while checking th IDs in DB Tables.' TYPE 'E'.
  ENDIF.
ENDFORM.
